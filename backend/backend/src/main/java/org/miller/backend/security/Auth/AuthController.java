...
public class AuthController {
    ...
    public ResponseEntity<?> register(@RequestBody RegisterRequest request) {
        try {
            ...
            return ResponseEntity.badRequest().body(Map.of("success", false, "message", "Invalid input: " + e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(500).body(Map.of("success", false, "message", "An error occurred: " + e.getMessage()));
        }
    }
}
